...
//			state:0인 상태에서 쿼리실행하면 state는 1로 변경 
			...
			if (updateCount > 0) {
//			Rental insert query 실행되면 rentalIdv insert query 실행 
				String rentalIdv_sql = "insert into rentalIdv (renIdvNum, memIndex, bookNum, libCode, renFlag, renIdvDelFlag";
...
//				state:0 인 상태에서 쿼리 실행시 'brw', 대출날짜, 대출만료일, 1인 상태: 'rev', 예약날짜, 예약 만료일 
			...
			//	System.out.println(rentalIdv_sql);
	...
	public int returnState(int memIndex, String bookNum, String libCode, String state) {
		...
		try {
			...
			if(updateCount>0) {
				pstmt = con.prepareStatement("update rentalIdv set renReturnDate = now(), renIdvDelFlag = 1 where bookNum = ?");
				pstmt.setString(1, bookNum);
				pstmt.executeUpdate();
			}
		} catch (SQLException e) {
...
//			System.out.println(rental_sql);
	...
	public int revCancelState(int memIndex, String bookNum) {
		...
		try {
			...
			if(updateCount>0) {
				pstmt = con.prepareStatement("update rentalIdv set renIdvDelFlag = 1, renFlag = 'brw' where bookNum = ?");
				pstmt.setString(1, bookNum);
				pstmt.executeUpdate();
			}
		} catch (SQLException e) {
